...
This is a sample template for sam-s3-lambda-app - Below is a brief explanation of what we have generated for you:
...
```bash
sam logs -n HelloWorldFunction --stack-name sam-s3-lambda-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-s3-lambda-app
```
